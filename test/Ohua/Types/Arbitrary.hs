...
instance Arbitrary Binding where
    arbitrary = do
        ...
        pure (unsafeMake $ toText $ a : as)
          `suchThat` (not . flip HS.member keywords . unwrap)
      where
        idstartchars = ['a' .. 'z'] <> ['A' .. 'Z']
        idchars = idstartchars <> ['0' .. '9'] <> ['_']
        keywords = HS.fromList ["let", "if", "then", "else", "in", "module", "import", "sf", "algo"]
instance Arbitrary QualifiedBinding where arbitrary = QualifiedBinding <$> arbitrary <*> arbitrary
instance Arbitrary SomeBinding where arbitrary = oneof [ Qual <$> arbitrary, Unqual <$> arbitrary]
instance Arbitrary NSRef where arbitrary = genFromMake `suchThat` (not . null . unwrap)
...
instance Arbitrary Assignment where
    arbitrary = oneof [Direct <$> arbitrary, Destructure <$> (arbitrary `suchThat` ((/= 1) . length))]
instance Arbitrary a => Arbitrary (Expr a) where
    ...
    shrink (Let _ a b)  = a:b: shrink b
    shrink (Apply a b)  = a:b:shrink a
